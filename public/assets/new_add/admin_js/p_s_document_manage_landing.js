...
function get_ambassadors(type,append_table){ 
    ...
    if(type=="Completed"){
        ...
        table = $('#'+append_table).DataTable({
            ...
            ajax: {
                url: get_p_s_admin_alumni_datatable, 
                type: 'POST', 
                data: function (d) {
...
function initTable(tableId,nester_tbl_u_id, data) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var emp_details = '<tr><td>'+data[0].emp_id+'</td><td>'+data[0].emp_name+'</td><td>'+data[0].pan_no+'</td><td>'+data[0].dob+'</td><td>'+data[0].remark+'</td><td>'+data[0].mobileno+'</td><td>'+data[0].email+'</td></tr>';
